...
// Processes image data in background thread for better performance
...
self.onmessage = function(e) {
    ...
    // Use Float32Array for error accumulation (more precise)
    ...
    // XTH mode uses specific thresholds for Xteink LUT
    // Standard mode uses linear quantization
    let quantize;
    if (xthMode) {
        // XTH: 4 levels with specific thresholds matching Xteink LUT
        // 0=White(255), 1=DarkGray(85), 2=LightGray(170), 3=Black(0)
        // Thresholds: 212, 127, 42
        quantize = (val) => {
    ...
    // Transfer back (using transferable for zero-copy)
